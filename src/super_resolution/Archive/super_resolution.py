...
from src.super_resolution.Archive.RelaxationLabelingStrategy import RelaxationLabelingStrategy


def calc_initial_confidence(scaled_image_shape,base_pixels,n_buckets):
    """
    calculates the initial confidence for each pixel in the image
    :param scaled_image_shape:
    :param base_pixels:
    :param n_buckets:
    """
...
def relaxation_labeling(image, base,scale_factor,n_buckets,epsilon):
    scaled_image,base_pixels = image, base# upscale(image, scale_factor)
    # # ____
    # kernel =  np.ones((5, 5))
    # linear kernel:
...
def run_super_resolution(upscaling_strategy, input_image_path, scale_factor,n_buckets,epsilon):
    ...
    processed_image = relaxation_labeling(scaled_image, base_pixels,scale_factor,n_buckets,epsilon) # img after relaxation labeling
